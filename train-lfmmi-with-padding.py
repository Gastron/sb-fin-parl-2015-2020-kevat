...
class LFMMIAM(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        normalized = self.modules.normalize(batch.feats.data, lengths=batch.feats.lengths)
        s = normalized.shape
        ends = (s[1]*batch.feats.lengths).long()
        # Now make a padded tensor:
        padded = torch.zeros((s[0], s[1] + 2*self.hparams.contextlen, s[2])).to(self.device)
        for i in range(s[0]):
            padded[i,0:self.hparams.contextlen,:] = normalized[i,0,:]
            padded[i,self.hparams.contextlen:self.hparams.contextlen+ends[i],:] = \
                normalized[i,0:ends[i],:]
            padded[i,self.hparams.contextlen+ends[i]:,:] = \
                normalized[i,ends[i]-1,:]

        encoded_all = self.modules.encoder(padded)
        first_relevant = int(self.hparams.contextlen / self.hparams.subsampling)
        last_relevant = first_relevant + int(s[1] / self.hparams.subsampling)
    ...
    def compute_objectives(self, predictions, batch, stage):
        ...
        output_lengths = (predictions.shape[1] * batch.feats.lengths).int().cpu()
        ...
        loss = self.hparams.chain_loss(predictions, output_lengths, numerator_graphs)
        return loss
