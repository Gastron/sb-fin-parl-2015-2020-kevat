    ...
    with open(hparams.test_probs_out, 'wb') as fo:
        with torch.no_grad():
            for uttid, feats in tqdm.tqdm(kaldi_io.read_mat_scp(hparams.test_feats), total=num_utts):
                ...
                normalized = modules.normalize(feats, lengths=torch.tensor([1.]))
                encoded_all = modules.encoder(normalized)
                out = modules.lin_out(encoded_all)
                if getattr(hparams, "apply_logsoftmax", False):
                    lprobs = hparams.log_softmax(out)
                else:
                    lprobs = out
                kaldi_io.write_mat(fo, lprobs.squeeze(0).cpu().numpy(), key=uttid)
